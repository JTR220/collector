...
>
	<div class="mx-auto max-w-7xl px-4 py-8 md:px-8">
		...
		<div class="mb-8 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
			<div>
				<p class="inline-flex rounded-full border border-orange-300 bg-orange-100 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-orange-700">
					Pilotage
				</p>
				<h1 class="mt-2 text-4xl font-black uppercase tracking-tight text-slate-900">
					Tableau de bord
				</h1>
			...
			<button
				...
				class="self-start rounded-2xl border border-slate-200 bg-white px-5 py-2 text-xs font-bold uppercase tracking-widest text-slate-700 shadow-sm transition hover:bg-slate-50 sm:self-auto"
			>
				Rafraîchir
			</button>
		</div>
				...
				{#each actors as actor}
					...
					>
						<div class="flex items-center gap-2">
							...
							<span class={`text-sm font-black uppercase tracking-wide ${activeActor === actor.id ? 'text-orange-700' : 'text-slate-800'}`}>
								{actor.label}
							</span>
						</div>
		...
		{#if activeActor === 'pm'}
			...
			<div class="mb-6 grid gap-4 lg:grid-cols-2">
				...
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="mt-4 h-3 overflow-hidden rounded-full bg-slate-100">
						<div class="h-full rounded-full bg-orange-500 transition-all" style="width: {avancement}%"></div>
					</div>
					...
					<ul class="mt-4 grid grid-cols-2 gap-1">
						{#each roadmapItems as item}
							<li class="flex items-center gap-2 text-xs text-slate-600">
								<span class={`h-2 w-2 flex-shrink-0 rounded-full ${item.done ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
								{item.label}
							</li>
						{/each}
					</ul>
						...
						{#each velocite as s}
							<div>
								<div class="mb-1 flex justify-between text-xs">
									...
									<span class={`font-bold ${s.points < 20 ? 'text-red-600' : 'text-slate-900'}`}>{s.points} pts</span>
								</div>
			...
			<div class="mb-6 grid gap-4 md:grid-cols-3">
				...
				<!-- Taux de bugs -->
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Taux de bugs</p>
					<div class="mt-3 flex items-end gap-3">
						<p class="text-4xl font-black text-slate-900">{bugRate.toFixed(2)}</p>
						<span class="mb-1 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">Moyen</span>
					</div>
					...
					<div class="mt-4 space-y-2 text-xs text-slate-600">
						<div class="flex justify-between rounded-xl bg-slate-50 px-3 py-2">
							<span>Bugs critiques ouverts</span><span class="font-bold text-red-600">{bugsOuverts}</span>
						</div>
						<div class="flex justify-between rounded-xl bg-slate-50 px-3 py-2">
				...
				<!-- Fix Rate -->
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Fix Rate</p>
					<div class="mt-3 flex items-end gap-3">
						<p class="text-4xl font-black text-slate-900">{fixRate}%</p>
						<span class="mb-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-700">Bon</span>
					</div>
					...
					<p class="mt-2 text-xs text-slate-400">Cible cours : &gt; 90% = bon · 70–90% = moyen · &lt; 70% = critique</p>
				</div>

				<!-- Couverture tests -->
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Couverture tests</p>
					<div class="mt-3 flex items-end gap-3">
						<p class="text-4xl font-black text-slate-900">{testCoverage}%</p>
						<span class={`mb-1 rounded-full px-2 py-0.5 text-xs font-bold bg-${cvg.color}-100 text-${cvg.color}-700`}>
							{cvg.label}
						</span>
					</div>
					<div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
						<div class={`h-full rounded-full bg-${cvg.color}-500`} style="width: {testCoverage}%"></div>
					</div>
			...
			<div class="rounded-3xl border border-orange-200 bg-orange-50 p-5">
				<p class="text-xs font-bold uppercase tracking-widest text-orange-700">Lecture PM · Cours p.17</p>
				<p class="mt-2 text-sm text-orange-800">
					Le PM suit l'avancement (prévu vs réel), la vélocité pour détecter les blocages et la qualité (bug rate, fix rate, couverture). Une chute de vélocité = signal d'alerte → réallouer ressources ou geler de nouvelles features.
				</p>
			</div>
		...
		{:else if activeActor === 'cto'}
			...
			<div class="mb-6 grid gap-4 lg:grid-cols-2">
				...
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<p class="mt-1 text-xs text-slate-400">Formule : temps fonctionnement / temps total × 100</p>
					<div class="mt-4 space-y-4">
						{#each [
							{ label: 'auth-service', status: authStatus },
							{ label: 'catalog-service', status: catalogStatus }
						] as svc}
							<div class="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
								...
								<span class={`rounded-full px-3 py-1 text-xs font-bold uppercase ${
									svc.status === 'ok' ? 'bg-emerald-100 text-emerald-700'
									: svc.status === 'loading' ? 'bg-slate-100 text-slate-500'
				...
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="mt-4 space-y-4">
						{#if catalogLatencyMs !== null}
							<div class="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
								<div>
									...
									<p class="text-xs text-slate-400">catalog-service /health · Formule : Σtemps / nb requêtes</p>
								</div>
								<span class={`rounded-full px-3 py-1 text-xs font-bold ${
									catalogLatencyMs < 200 ? 'bg-emerald-100 text-emerald-700'
									: catalogLatencyMs < 500 ? 'bg-amber-100 text-amber-700'
									: 'bg-red-100 text-red-700'
								}`}>
									{catalogLatencyMs} ms
								</span>
						...
						<div class="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
							...
							<span class={`rounded-full px-3 py-1 text-xs font-bold ${
								catalogStatus === 'ok' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
							}`}>
								{catalogStatus === 'ok' ? '< 1%' : '...'}
							</span>
						</div>
			...
			<div class="mb-6 rounded-3xl border border-slate-800 bg-slate-950 p-6 text-white shadow-lg">
				<p class="mb-1 text-xs font-semibold uppercase tracking-widest text-orange-300">
					State of DevOps Report — 4 indicateurs clés
				</p>
				<h2 class="mb-5 text-xl font-black uppercase tracking-tight">DORA Metrics</h2>
				<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
					{#each doraMetrics as metric}
						<div class="rounded-2xl border border-slate-800 bg-slate-900 p-4">
							...
							<span class={`mt-3 inline-block rounded-full px-2 py-0.5 text-xs font-bold ${
								metric.status === 'good' ? 'bg-emerald-900 text-emerald-400'
								: metric.status === 'medium' ? 'bg-amber-900 text-amber-400'
								: metric.status === 'bad' ? 'bg-red-900 text-red-400'
								: 'bg-slate-800 text-slate-400'
							}`}>
								{metric.status === 'good' ? 'Élite' : metric.status === 'medium' ? 'En progression' : metric.status === 'na' ? 'À mesurer' : 'Critique'}
							</span>
						</div>
					{/each}
				</div>
			...
			<div class="rounded-3xl border border-orange-200 bg-orange-50 p-5">
				<p class="text-xs font-bold uppercase tracking-widest text-orange-700">Lecture CTO · Cours p.17</p>
				<p class="mt-2 text-sm text-orange-800">
					Le CTO suit la disponibilité des services, la latence et les métriques DORA. Le MTTR et le Change Failure Rate nécessiteront Prometheus / Grafana en production pour être mesurés avec précision.
				</p>
			</div>
		...
		{:else if activeActor === 'ceo'}
				...
				{#each [
					{
						label: 'Taux de churn',
						value: churnRate,
						sub: 'Formule : utilisateurs perdus / total × 100',
						threshold: '< 5% = bon · > 10% = critique',
						color: 'slate'
					},
					{
						label: 'Taux de rétention',
						value: retentionRate,
						sub: 'Utilisateurs actifs après période / initiaux × 100',
						threshold: 'Fidélité produit',
						color: 'slate'
					},
					{
						label: 'Taux de conversion',
						value: conversionRate,
						sub: 'Visiteurs ayant agi / visiteurs totaux × 100',
						threshold: 'Cible : > 5%',
						color: 'slate'
					},
					{
						label: 'ARPU',
						value: arpu,
						sub: 'Revenu total / nombre d\'utilisateurs',
						threshold: 'Amélioration UX → ARPU augmente',
						color: 'slate'
					}
				] as kpi}
					<div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
						<p class="text-xs font-semibold uppercase tracking-widest text-slate-400">{kpi.label}</p>
			...
			<div class="mb-6 grid gap-4 md:grid-cols-2">
				<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					<p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Articles publiés</p>
			...
			<div class="mb-6 rounded-3xl border border-amber-200 bg-amber-50 p-5">
				<p class="text-xs font-bold uppercase tracking-widest text-amber-700">KPIs business — À instrumenter</p>
				<p class="mt-2 text-sm text-amber-800">
					Churn, rétention, conversion et ARPU nécessitent un outil d'analytics dédié (Amplitude, Mixpanel, ou tracking custom). Ces indicateurs sont définis dans le cours mais non encore mesurables sans collecte de données utilisateurs.
				</p>
			</div>

			<div class="rounded-3xl border border-orange-200 bg-orange-50 p-5">
				<p class="text-xs font-bold uppercase tracking-widest text-orange-700">Lecture CEO · Cours p.12-14</p>
				<p class="mt-2 text-sm text-orange-800">
					Le CEO suit la valeur business créée : rétention, churn, conversion, ARPU. Un projet techniquement bon mais business KO = échec. Lien direct : KPI technique dégradé → KPI business impacté (ex. latence élevée → churn augmente).
				</p>
			</div>
